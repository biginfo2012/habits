...
import { ArchiveIcon, ChartBarIcon, ChartPieIcon, ChevronRightIcon, ClipboardListIcon, CogIcon, CreditCardIcon, DocumentReportIcon, InboxIcon, InformationCircleIcon, LocationMarkerIcon, NewspaperIcon, TagIcon, TableIcon, TemplateIcon, TicketIcon, UsersIcon, ViewGridIcon, XIcon, ChipIcon } from "@heroicons/react/solid";
import { LogoutIcon } from "@heroicons/react/outline";
import { Nav, Badge, Image, Button, Dropdown, Navbar, Collapse, Col } from 'react-bootstrap';
import { Link } from 'react-router-dom';

import { Routes } from "routes";
import ReactHero from "assets/img/habits_logo.png";
  ...
  return (
    <>
      <Navbar as={Col} xs={12} expand={false} collapseOnSelect variant="dark" className="navbar-theme-primary px-4 d-lg-none">
        <Navbar.Brand as={Link} to={Routes.Home.path} className="me-lg-5">
      ...
      <CSSTransition timeout={300} in={show} classNames="sidebar-transition">
        <SimpleBar {...events} className={`${contracted} ${showClass} sidebar d-lg-block bg-gray-800 text-white collapse`}>
          <div className="sidebar-inner sidebar-hide-scrollbars px-4 pt-3">
            ...
            <Nav className="flex-column pt-3 pt-md-0">
              <NavItem title="Habits" link={Routes.Home.path} image={ReactHero} />
              ...
              {userInfo.role === 'admin' && <NavItem title="Brands" icon={TicketIcon} link={Routes.Brands.path} />}
              {userInfo.role === 'admin' && <NavItem title="Users List" icon={UsersIcon} link={Routes.Users.path} />}
              {userInfo.role === 'brand' && <NavItem title="Products" icon={ArchiveIcon} link={Routes.Products.path} />}
              {userInfo.role === 'brand' && <NavItem title="NFTs" icon={ChipIcon} link={Routes.NFTs.path} />}
              ...
              {/* <NavItem title="Settings" icon={DocumentReportIcon} link={Routes.Settings.path} /> */}
